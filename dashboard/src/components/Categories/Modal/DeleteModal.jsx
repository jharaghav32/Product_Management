...
import { deleteCategory } from '../../../Feature/Fetch'
const DeleteModal = ({id,handleClose,deletecategory,setdeletecategory}) => {
  ...
  return (
    ...
    style={{zIndex:'250'}}
    className={`fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform ${deletecategory?'transform-none':'translate-x-full'} bg-white dark:bg-gray-800`}
    tabindex="-1"
...
>
    ...
    <button
        ...
        onClick={()=>handleClose(setdeletecategory)}
    >
        <svg
            aria-hidden="true"
            className="w-5 h-5"
